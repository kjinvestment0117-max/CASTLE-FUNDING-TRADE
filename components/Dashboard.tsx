...
const Dashboard: React.FC<{ plan: PricingPlan | null }> = ({ plan }) => {
  const { t } = useLanguage();
  const accSize = plan ? plan.accountSize : '$100,000';
    ...
    <div className="flex min-h-screen bg-[#050505]">
      ...
      <aside className="w-80 bg-[#050505] border-r border-white/5 hidden lg:flex flex-col p-8 space-y-12 shrink-0">
        <a href="#home" className="flex items-center space-x-3">
          <div className="w-10 h-10 btn-gold rounded-xl flex items-center justify-center font-black text-black shadow-lg">C</div>
        ...
        <div className="glass p-8 rounded-[40px] border border-[#b8860b]/20 text-center space-y-5 relative overflow-hidden group">
          ...
          <div className="text-2xl font-castle font-black castle-gradient uppercase italic tracking-widest">PHASE 1</div>
          <div className="h-px bg-white/5 w-full"></div>
          <button className="w-full py-3 bg-white/5 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest hover:bg-[#b8860b] hover:text-black transition-all">Evaluation Policy</button>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6 lg:p-12 space-y-12 overflow-y-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="space-y-2">
            <h1 className="text-4xl font-castle font-black uppercase tracking-tighter italic">{t.mypage.title}</h1>
            <div className="flex items-center space-x-3">
              <span className="text-gray-500 font-bold uppercase text-[10px] tracking-widest">Active Challenge: <span className="text-[#b8860b]">{accSize} KING CHALLENGE</span></span>
              <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
            </div>
          </div>
          <div className="flex space-x-4">
            <button className="px-6 py-4 glass border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-white/5 transition-all">Analytics</button>
            <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: t.dashboard.gauges.profit, value: '75%', color: 'btn-gold', current: '$7,500', goal: '$10,000' },
            { label: t.dashboard.gauges.drawdown, value: '8%', color: 'bg-red-500/50', current: '$92,000', goal: '$90,000' },
            { label: t.dashboard.gauges.daily, value: '15%', color: 'bg-orange-500/50', current: '$98,500', goal: '$95,000' },
            { label: t.dashboard.gauges.days, value: '100%', color: 'bg-green-500/50', current: '5/5 Days', goal: 'Completed' },
          ].map((g) => (
            <div key={g.label} className="glass p-8 rounded-[32px] border border-white/5 space-y-6 transition-transform hover:scale-[1.03] relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                <span className="text-2xl">📊</span>
              </div>
              <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{g.label}</div>
              <div className="space-y-3">
                <div className="text-2xl font-castle font-black text-white/90">{g.current}</div>
                <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                  <div className={`h-full ${g.color} transition-all duration-1000 ease-out`} style={{ width: g.value }}></div>
                </div>
                <div className="text-[9px] font-bold text-gray-500 uppercase tracking-widest flex justify-between">
                  <span>Progress</span>
                  <span>{g.goal}</span>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 glass p-10 rounded-[48px] border border-white/5 h-[500px] relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-12 relative z-10">
              <h3 className="text-2xl font-castle font-black uppercase italic castle-gradient tracking-tighter">{t.mypage.charts.equity}</h3>
          ...
          <div className="space-y-8">
            <div className="glass p-10 rounded-[48px] border border-white/5 space-y-8 relative overflow-hidden">
              <h3 className="text-xl font-castle font-black uppercase italic tracking-tight">System Node</h3>
              <div className="space-y-5">
                <div className="p-6 bg-white/5 rounded-3xl border border-white/5 group hover:border-[#b8860b]/30 transition-all">
                  <div className="text-[9px] font-black text-gray-500 uppercase mb-2 tracking-widest">{t.dashboard.account.id}</div>
                  <div className="font-mono text-[#b8860b] text-xl font-black">CT_9921_SIM</div>
                </div>
                <div className="p-6 bg-white/5 rounded-3xl border border-white/5 group hover:border-[#b8860b]/30 transition-all">
                  <div className="text-[9px] font-black text-gray-500 uppercase mb-2 tracking-widest">{t.dashboard.account.platform}</div>
                  <div className="font-bold text-white uppercase text-xl tracking-tight">MetaTrader 5</div>
                </div>
                <div className="p-6 bg-[#b8860b]/5 rounded-[32px] border border-[#b8860b]/10 text-center">
                  <div className="text-gray-400 text-[10px] font-bold uppercase tracking-widest italic mb-2">Payout Ratio</div>
                  <div className="text-3xl font-black castle-gradient">90% Split</div>
                </div>
              </div>
            </div>
          </div>
